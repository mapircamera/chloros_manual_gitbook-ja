...
校正済み画像をChlorosで一般的なJPG形式、またはTIFFで出力する場合、反射率パーセントはピクセル値を画像フォーマットのビット深度で割ることで算出されます。つまりJPGでは255で、TIFFでは65,535で割ります。 Chlorosでパーセント形式出力を選択することも可能です。この場合、各ピクセルは0.0～1.0のパーセント値（0%～100%の反射率）の範囲になります。ただし、一部の画像アプリケーションはパーセント（浮動小数点）形式の画像に対応しておらず、またストレージ容量も大きくなる点にご注意ください。
