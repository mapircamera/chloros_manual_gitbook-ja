...
インデックス/LUTサンドボックスは、Chloros Image Viewer内に設けられたインタラクティブな作業領域です。マルチスペクトルインデックスの計算やカラー可視化をリアルタイムで試行できます。この強力なツールにより、異なるインデックスのテスト、値範囲の微調整、データセット全体の再処理なしでの出版物対応可視化の作成が可能です。
...
* **インタラクティブなLUT調整** - 色のグラデーションと範囲を微調整
...
* **可視化設定をインタラクティブに調整*** **事前に計算済みのインデックス画像を表示*** **全ズームレベルでピクセル値を検査**

### サンドボックスの起動

インデックス/LUTサンドボックスは、**画像ビューア** <img src="../.gitbook/assets/icon_image-viewer.JPG" alt="" data-size="line"> サイドバータブからアクセスします：

1. ファイルブラウザの画像グリッドで画像をクリックすると、**画像ビューアー**タブが開きます <img src="../.gitbook/assets/icon_image-viewer.JPG" alt="" data-size="line"> タブで開きます
2. **イメージビューア** <img src="../.gitbook/assets/icon_image-viewer.JPG" alt="" data-size="line"> タブをクリックすると、左側のポップアウトサイドバーが開きます（未開の場合）
...
* **出力**: RGB カラー（例：明るい緑）
* **目的**: パターンを視認・解釈しやすくする**グレースケール vs カラーLUT:**

* グレースケール: 科学的かつ中立的、生データを表示
* カラーLUT: 直感的かつインパクト大、パターンや差異を強調
...
1. <img src="../.gitbook/assets/image (1) (1).png" alt="" data-size="line"> 「+LUT追加」ボタンをクリック
2. カラーグラデーションを選択
3. クリッピングの最小/最大終端点を調整
4. クリッピングモードを調整
5. **画像ビューアー** <img src="../.gitbook/assets/icon_image-viewer.JPG" alt="" data-size="line"> タブのサイドバーで「インデックス」チェックボックスをオンにしてLUTを適用
...
**クラス制御**は、グラデーションに表示される離散的な色の段階数を決定します：**クラス数のオプション：*** **2～5クラス**：非常に広いカテゴリー、明確なゾーン
* **6～10クラス**：バランスが良く、分類に適する
* **11～20クラス**：滑らかなグラデーション、連続的な見た目
* **20クラス以上**：ほぼ連続的、最大限の滑らかさ**調整方法：**

1. LUTパネルで、**グラデーションバー下部のカラースウォッチ**を探します
2. +ボタンでクラス数を追加して調整します
3. カラースウォッチをダブルクリックするとクラス数を減らせます
4. グラデーションは画像上で**リアルタイムで更新**されます**可視化への効果:*** **少ないクラス数** (3-5): 明確な領域分け、分類の簡素化、カテゴリの識別容易化
* **中程度のクラス数** (6-10): バランスの取れたアプローチ、大半の用途に適す
* **多いクラス数** (15-20): 滑らかな遷移、詳細な変化、写真のような外観**使用シーン:*** **少ないクラス (3-5)**: プレゼンテーションスライド、分類マップ、簡易レポート
* **中程度のクラス (6-10)**: 一般的な分析、バランスの取れた詳細、標準レポート
* **多くのクラス (15-20)**: 科学的な分析、詳細な検査、出版物レベルの出力

### 値範囲の微調整

**値範囲コントロール**は、グラデーション内のどのインデックス値がどの色にマッピングされるかを決定します：**LUTパネル内の範囲コントロール：*** **最小値**：カラースケールの下限
* **最大値**：カラースケールの上限
...
5. **最大値**フィールドでも同様に入力（例：`0.9`）
6. ビジュアライゼーションが**即時更新**されます

{% hint style=&quot;info&quot; %}
**自動スケーリング**: LUTを初めて適用すると、Chlorosが画像内の実際のデータ範囲に基づいて最小値/最大値を自動的に設定します。その後、この範囲を絞り込み、特定の関心値範囲に焦点を当てることができます。
{% endhint %}

**範囲調整の例:*** **全範囲**: `-1.0` から `1.0` (全値を表示)
* **植生に焦点を当てた範囲**: `0.2` から `0.9` (裸地と水域を除外)
* **健全な植生のみ**: `0.5` から `0.9` (活発な植物のみを強調)
* **ストレス検出**: `0.2` から `0.5` (問題領域を強調)
* **カスタム範囲**: 観測されたピクセル値に基づいて調整**範囲調整の目的*** 関心領域の**コントラスト向上*** **無関係な値の除外**（例：水域、裸地）
* 複数画像や日付間の**可視化標準化*** 狭い値範囲内の**微妙な差異の強調**

### 範囲外値のクリッピング

定義した最小/最大範囲外のピクセル値は、**クリッピングモード**で表示方法を制御できます。
...
* **最大値を超える**ピクセル → グラデーションの**最終色**（例：緑）で表示
* **使用例**：極端な値を強調、限界値で飽和した色を用いて全データ範囲を表示
* **例**：NDVIの値が0.2未満は全て赤、0.9超は全て緑で表示

#### 2. 透明背景

* **範囲外のピクセル**は**完全に透明化*** **範囲内のピクセル**のみが色グラデーションを表示
* **使用例**: GISオーバーレイ、特定値範囲の分離、関心領域のみの強調表示
* **例**: NDVI 0.4-0.7の範囲のみをカラー表示、それ以外は透明化
...
* **例**: ストレスを受けた植生をカラー強調（NDVI 0.3-0.5）し、健全な領域をグレー表示
...
| -------------------------- | ------------------------------------------ | ---------------------------- |
| **最小値と最大値**    | データ全表示、科学分析     | 全ピクセル着色           |
| **透明背景**    | GISオーバーレイ、特定範囲の分離    | 範囲内着色、範囲外空白 |
| **インデックス背景**       | 控えめな強調、データ文脈維持  | 範囲内着色、範囲外グレー  |
| **オリジナル背景**    | レポート、プレゼンテーション、直感的分析 | 範囲内はカラー、範囲外は写真 |

### カスタムLUTカラーの作成

可視化を完全に制御するには、個々のカラーストップを編集して**カスタムカラーグラデーション**を作成できます。**カスタムグラデーションの作成方法:**

1. LUTパネルで**グラデーションプレビューバー**を探します
2. グラデーション下部の**カラースウォッチ**を探す
3. **カラーストップをクリック**して選択
4. **カラーピッカー**が開く
5. 以下の方法で新色を選択：
   * **カラーホイール**：視覚的色選択
   * **RGB/HSVスライダー**：精密な色制御
   * **16進コード入力**： 正確な色指定（例：赤の場合は赤）
6. カラーピッカーの外側をクリックして**新しい色を適用**

7. 画像上のグラデーションが**即座に更新**されます**カラーストップの追加または削除:*** **ストップの追加**: +アイコンをクリックし、末尾に新しいスウォッチを追加
* **ストップの削除**: 色の四角をダブルクリックしてスウォッチを削除**カスタマイズ戦略:*** **グラデーションの反転**: 色順を反転させて意味を逆転（例: 緑=低、赤=高）
* **ブランドカラー**: レポート用に組織のカラーパレットに合わせる
* **色覚障害対応**: オレンジ-青または紫-黄色の組み合わせを使用
* **印刷最適化**: カラー印刷とグレースケール印刷の両方で効果的な色を選択
* **複数閾値**: 分類用に特定の値閾値で異なる色を使用

{% hint style=&quot;info&quot; %}
**カスタムグラデーションの保存**: カスタムグラデーションは保存・再利用可能です。LUTパネルの保存アイコンをクリックし、カスタムカラースキームを将来使用するために保存してください。
{% endhint %}
...
サンドボックス内のすべてのLUT調整は、画像を**即座にインタラクティブに**更新します：
...
* **クリッピング変更** → 背景表示が即時変更
* **カラー編集** → カスタムグラデーションが即時適用**「適用」ボタン不要** - 全ての変更がライブかつインタラクティブに反映！
...
6. **クリッピングを選択** - コンテキスト確認のため「オリジナル背景」を試す
7. **色を微調整** - 特定強調が必要な場合、グラデーションをカスタマイズ
8. **設定を確定** - 設定を記録し、エクスポート処理用にプロジェクト設定へコピー

### ピクセル値の確認

効果的なLUT範囲設定には実際のピクセル値の理解が不可欠です：

**値の確認方法：**

1. ピクセル値は、画像で「インデックス」ボックス、または「インデックス」と「LUT」の両方のボックスが**チェックされている**場合に表示されます。
2. **カーソルを**画像の異なる領域に**移動**

3.**ホバー時に表示される**レジェンドの**ピクセル値を確認**
...
5. 異なる特徴の値範囲を**記録**します：
   ...
   * **裸地**：例 NDVI 0.05-0.25
   * **水域**（存在する場合）：例 NDVI -0.05～0.10**ピクセル値を用いたLUT範囲の設定:**ピクセル値を確認後、LUTの最小値/最大値を適宜調整してください:**例:*** **観測結果**: 土壌値 = 0.05-0.25、ストレス状態 = 0.25-0.50、健全状態 = 0.50-0.85
* **目標**：植物の健康状態のみ可視化（土壌を除く）
* **LUT設定**：最小値 = `0.25`、最大値 = `0.85`
* **クリッピング**: 「オリジナル背景」を選択し、土壌を自然な色で表示
* **結果**: カラーグラデーションは植生のみに適用され、土壌は元の画像のまま表示

{% hint style=&quot;info&quot; %}
**ダイナミックレンジ**: 作物、季節、生育段階によって値の範囲が異なります。LUT範囲を設定する前に、必ず対象データセットのピクセル値を確認してください。
{% endhint %}
...
{% hint style=&quot;info&quot; %}
**作成場所**: カスタムインデックスは、処理前の**プロジェクト設定**およびイメージビューアーのサンドボックスサイドバーで設定できます。
{% endhint %}

**カスタムインデックスの作成方法:**

1.**プロジェクト設定を開く**（処理前）またはイメージビューアーのサンドボックスサイドバー
2. **インデックス式ドロップダウン**に移動
