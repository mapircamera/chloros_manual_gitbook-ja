...
インデックス/LUTサンドボックスは、Chloros Image Viewer内のインタラクティブな作業領域であり、マルチスペクトルインデックス計算とカラー可視化をリアルタイムで実験できます。この強力なツールにより、異なるインデックスのテスト、値範囲の微調整、データセット全体の再処理なしに出版可能な可視化の作成が可能です。
...
* グレースケール: 科学的・中立的、生データを表示
* カラーLUT: 直感的・インパクト大、パターンや差異を強調
...
4. クリッピングモードを調整
5. **画像ビューアー**タブのサイドバーで「インデックス」ボックスをチェックしLUTを適用

### カラーグラデーションの選択

**グラデーションの選択方法:** <img src="../.gitbook/assets/icon_image-viewer.JPG" alt="" data-size="line"> タブのサイドバーで「インデックス」チェックボックスをオンにしてLUTを適用
...
3. カラースウォッチをダブルクリックしてクラス数を減らします
4. グラデーションは画像上で**リアルタイム**に更新されます

**視覚化への影響:**

* **少ないクラス数** (3-5): 明確な領域の生成、分類の簡素化、カテゴリの識別容易化
* **中程度のクラス数** (6-10): バランス重視、大半の用途に適す
* **多数のクラス** (15-20): 滑らかな遷移、詳細な差異表現、写真のような外観

**使用タイミング:**

* **少数のクラス (3-5)**: プレゼンテーションスライド、分類マップ、簡易レポート
* **中程度のクラス数（6-10）**：一般的な分析、バランスの取れた詳細、標準的なレポート
* **多数のクラス（15-20）**：科学的分析、詳細な検査、出版物レベルの出力

### 値範囲の微調整

**値範囲コントロール**は、グラデーション内でどのインデックス値がどの色にマッピングされるかを決定します：
...
* **全範囲**: `-1.0` から `1.0` (全値を表示)
* **植生に焦点を当てた範囲**: `0.2` から `0.9` (裸地と水域を除外)
* **健全な植生のみ**: `0.5` から `0.9` (活発な植物のみを強調)
* **ストレス検出**: `0.2` ～ `0.5` (問題領域を強調)
* **カスタム範囲**: 観測されたピクセル値に基づいて調整
...
* **使用例**：極端な値を強調、限界値で飽和色を用いた全データ範囲の表示
* **例**：NDVI値が0.2未満は全て赤、0.9超は全て緑で表示

#### 2. 透明背景

* **範囲外のピクセル**は**完全に透明化**
* **範囲内のピクセル**のみが色グラデーションを表示
* **使用例**: GISオーバーレイ、特定値範囲の分離、関心領域のみの強調表示
* **例**: NDVI 0.4-0.7の範囲のみを色表示、それ以外は透明
...
* **例**: ストレスを受けた植生をカラー強調表示（NDVI 0.3-0.5）、健全な領域はグレー表示
...
| -------------------------- | ------------------------------------------ | ---------------------------- |
| **最小値と最大値**    | データ完全表示、科学分析     | 全ピクセル着色           |
| **透明背景** | GISオーバーレイ、特定範囲の抽出     | 範囲内はカラー、範囲外は空白 |
...
* **ストップ追加**: +アイコンをクリックし末尾に新しいスウォッチを追加
* **ストップ削除**: 色四角をダブルクリックしてスウォッチを削除
...
{% hint style=&quot;info&quot; %}
**カスタムグラデーションの保存**: カスタムグラデーションは保存・再利用可能。LUTパネルの保存アイコンをクリックし、カスタム配色を将来使用するために保存してください。
{% endhint %}
...
{% hint style=&quot;success&quot; %}
**ライブフィードバック**: 瞬時の視覚的フィードバックにより、分析ニーズに最適な可視化が見つかるまで様々な設定を迅速に試行できます。
{% endhint %}
...
3. **LUTグラデーションの適用** - Red-Yellow-Greenプリセットから開始
4. **ピクセル値の確認** - カーソルを移動させ、値の範囲を把握
5. **最小/最大値の調整** - 植生に焦点を絞るため範囲を狭める（例：0.2～0.9）
6. **クリッピングの選択** - コンテキスト把握のため「オリジナル背景」を試す
7. **色の微調整** - 特定強調が必要な場合、グラデーションをカスタマイズ
8. **設定を確定** - 設定を記録し、エクスポート処理用にプロジェクト設定へコピー

### ピクセル値の確認

効果的なLUT範囲を設定するには、実際のピクセル値を理解することが重要です：

**値の確認方法：**

1. ピクセル値は、画像の「インデックス」ボックス、または「インデックス」と「LUT」の両方のボックスが**チェックされている**場合に表示されます。
2. 画像の異なる領域に**カーソルを移動**
3. ホバー時に表示される**ピクセル値を観察**
...
ピクセル値を検証後、LUTの最小値/最大値を適宜調整してください:
...
* **結果**：色調グラデーションは植生のみに適用され、土壌は元の画像のまま表示

{% hint style=&quot;info&quot; %}
**ダイナミックレンジ**：作物、季節、生育段階によって値の範囲は異なります。LUT範囲を設定する前に、必ず対象データセットのピクセル値を確認してください。
{% endhint %}
