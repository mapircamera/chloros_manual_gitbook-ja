...
Chloros イメージビューアは、マルチスペクトル画像の表示、分析、操作のための専用全画面インターフェースを提供します。オリジナル画像の表示でも処理済み出力の表示でも、イメージビューアは検査と分析のための強力なツールを提供します。
...
1. **画像ビューア**内の**指標/LUTサンドボックス**を特定 <img src="../.gitbook/assets/icon_image-viewer.JPG" alt="" data-size="line"> サイドバー
...
1. 処理済み出力を表示しているか確認（ファイル名拡張子を確認）
2. パーセントモードボタンの状態を確認
3. 同一データセットの正常な画像と比較
