...
* [ ] **ファイルのインポート** - 全ての画像がファイルブラウザに表示されている
* [ ] **ターゲット画像のマーキング** - キャリブレーション画像のターゲット列にチェックが入っている
...
* 位置：ウィンドウ上部中央
* アイコン：**再生/開始ボタン** <img src="../.gitbook/assets/image (2).png" alt="" data-size="line">
...
Chlorosは、ライセンスに応じて2種類の処理モードで動作します：
...
4. **エクスポート** - 処理済み画像とインデックスを保存

**プログレスバー操作:**

* **マウスオーバー** で詳細な4段階ドロップダウンパネルを表示
* **クリック** でドロップダウンパネルを固定表示
* **再度クリック** で固定解除しパネルを非表示
...
* 設定されたマルチスペクトルインデックスを計算（NDVI、NDREなど）
* 較正済み画像にバンド演算を適用
...
* カメラモデル別サブフォルダにファイルを書き込み
* オリジナルファイル名をサフィックス付きで保持

**所要時間:** エクスポート形式とファイルサイズにより異なる
...
{% hint style=&quot;warning&quot; %}
**パフォーマンスのヒント**: 最適なChloros+のパフォーマンスを得るには、他のアプリケーションを閉じてChlorosがシステムリソースを完全に使用できるようにしてください。
{% endhint %}
...
1. **停止/キャンセルボタン**（処理中は開始ボタンと置換）を探す
2. 停止ボタンをクリックする
3. 処理が即時停止します
4. 部分的な結果は破棄されます
...
* 必要に応じて設定を調整する
* 処理を最初から再開する
* 最適な動作のためには、Chlorosを完全に終了し再起動

{% hint style=&quot;warning&quot; %}
**部分的な結果は存在しません**: キャンセルするとすべての進行状況が破棄されます。Chlorosは部分的に処理された画像を保存しません。
{% endhint %}
...
実際の処理時間は以下によって大きく異なります：

* 画像枚数
* 画像解像度
* RAW 対 JPG 入力形式
* 処理モード（Free 対 Chloros+）
* CPU速度とコア数
* GPU利用可否（Chloros+のみ）
* 計算するインデックス数
* エクスポート形式の複雑さ
...
| ----------- | --------- | -------------- | -------------- |
| 50枚   | 15-20分 | 5-8分        | 3-5分        |
| 100枚   | 30-40分 | 10-15分      | 5-8分        |
| 200枚   | 1-1.5時間 | 20-30分      | 10-15分      |
| 500枚  | 2-3時間   | 45-60分      | 20-30分      |
| 1000枚 | 4-6時間   | 1.5-2時間      | 40-60分      |

{% hint style=&quot;info&quot; %}
**初回実行**: Chlorosがキャッシュとプロファイルを構築するため、初期処理に時間がかかる場合があります。 類似データセットの次回以降の処理は高速化されます。
{% endhint %}
...
1. デバッグログでエラーメッセージを確認 <img src="../.gitbook/assets/icon_log.JPG" alt="" data-size="line"> でエラーメッセージを確認
2. 空きディスク容量を確認
3. より少数の画像で処理を試行
4. 画像が破損していないことを確認
...
2. **利用可能なディスク容量の確認** - データセットサイズの2～3倍の空き容量を確保
3. **不要なアプリケーションの終了** - システムリソースを解放
4. **ターゲット画像の確認** - マークしたターゲットをプレビューし品質を確認
5. **プロジェクトを保存** - 自動保存機能あり、手動保存も推奨

### 処理中

1. **システムスリープを回避** - 省電力モードを無効化
2. **Chlorosをフォアグラウンドに維持** - タスクバー表示状態を保つ
3. **進行状況を随時確認** - 警告やエラーをチェック
...
1. **進行状況を監視** - [処理の監視](monitoring-the-processing.md)を参照
2. **完了を待つ** - 処理は自動実行されます
